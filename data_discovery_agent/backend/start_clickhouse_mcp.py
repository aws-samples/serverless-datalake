...
logging_config = {
    ...
    'handlers': {
        ...
        'file': {
            ...
            'filename': 'logs/clickhouse_mcp.log',
            'mode': 'a',
        },
    },
    'loggers': {
...
if __name__ == "__main__":
    try:
        from database.mcp.clickhouse_mcp import main
        print("🚀 Starting ClickHouse MCP Server on http://localhost:8002/sse")
        print(f"📝 Logs will be written to: {os.path.abspath('logs/clickhouse_mcp.log')}")
        ...
        logger.info(f"Log file: {os.path.abspath('logs/clickhouse_mcp.log')}")
        
        main()
    except Exception as e:
        print(f"❌ Failed to start ClickHouse MCP Server: {e}")
        logger = logging.getLogger(__name__)
        logger.error(f"Failed to start ClickHouse MCP Server: {e}")
        sys.exit(1)