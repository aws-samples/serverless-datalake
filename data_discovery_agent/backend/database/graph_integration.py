...
if __name__ == "__main__":
    async def test_graph_system():
        ...
        print("Testing Graph Integration System")
        print("=" * 50)
        
        # Initialize
        success = await initialize_graph_system()
        if not success:
            print("❌ Failed to initialize graph system")
            return
        
        print("✅ Graph system initialized successfully")
        
        # Get system info
        info = get_graph_info()
        print(f"📊 System Status: {info['status']}")
        print(f"🤖 Available Agents: {info['agent_count']}")
        
        # Test queries
        test_queries = [
            "What is 15 * 23?",
            "Tell me about France",
            "Calculate 100/4 and verify the result"
        ]
        
        for query in test_queries:
            print(f"\n🔍 Testing query: {query}")
            result = await process_graph_query(query)
            print(f"📝 Status: {result['status']}")
            print(f"💬 Response: {result['response'][:100]}...")
        
        # Cleanup
        await graph_integration.cleanup()
        print("\n🧹 Cleanup completed")
